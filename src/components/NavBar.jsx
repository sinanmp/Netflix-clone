...
function NavBar({useLocation}) {
  ...
  return (
    <nav className="bg-transparent py-6 fixed w-full top-0 left-0 z-30">
      <div className="container mx-auto flex justify-between items-center">
        <div className="flex items-center">
          <Link to={'/'}>
          <img src={LogImg} alt="Netflix Logo" className="h-11 max-md:h-7 cursor-pointer mr-2 ml-4" />
          </Link>
        ...
        <div className='mr-2 flex gap-2'>
          {!user && (
           <>
            <button onClick={()=>  navigate('/signup')} className="bg-black text-white px-4 py-2 rounded-full focus:outline-none hover:bg-red-700">Signup</button>
